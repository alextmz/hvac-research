...
create or replace view public.trusted_claims as
select
  c.id,
  c.entity_id,
  c.subject_text,
...
where c.status in ('supported','verified')
  and not exists (
    select 1
    from public.claim_integrity_quarantine q
...
create or replace function public.begin_research_run(
  ...
  p_stale_after interval default '00:30:00'::interval
)
returns uuid
language plpgsql
set search_path to ''
...
begin
  ...
  if v_bad > 0 then
    raise exception 'integrity gate failed: % unquarantined effective supported/verified claims lack supporting evidence', v_bad;
  end if;
  ...
  values(
    ...
    'Started through effective-claim integrity-gated begin_research_run()'
  );
